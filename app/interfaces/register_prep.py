...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    def __init__(self, main_app, preptype="", prep_data=None):
        ...
        self.output_connections.grid(row=0, column=0, columnspan=2, sticky="w", padx=2.5, pady=5)

        self.label_out = ctk.CTkLabel(self.output_connections, text="Output Connections", text_color="white")
        ...
        self.password_out = ctk.CTkEntry(self.output_connections, width=197.5, placeholder_text="Password Output")
        ...
        self.input_connections.grid(row=0, column=2, columnspan=2, sticky="nsew", padx=2.5, pady=5)

        self.label_in = ctk.CTkLabel(self.input_connections, text="Input Connections", text_color="white")
        ...
        self.password_in = ctk.CTkEntry(self.input_connections, width=197.5, placeholder_text="Password Input")
        ...
        if self.prep_data:
            ...
            self.parameters_list = self.prep_data["parameters"].split(",") if self.prep_data["parameters"] else []
            parameters = dict(item.split(":", 1) for item in self.parameters_list if ":" in item)
            # Insere no entrys os dados já cadastrados de Output
            self.username_out.insert(tkinter.END, parameters.get("UsernameOut"))
            passwordOut = Hash().restore_password(parameters.get("PasswordOut"))
            self.password_out.insert(tkinter.END, passwordOut)
            self.server_url_out.insert(tkinter.END, parameters.get("ServerURLOut"))
            self.content_url_out.insert(tkinter.END, parameters.get("ContentOut"))
            # Insere no entrys os dados já cadastrados de Input
            self.username_in.insert(tkinter.END, parameters.get("UsernameIn"))
            passwordIn = Hash().restore_password(parameters.get("PasswordIn"))
            self.password_in.insert(tkinter.END, passwordIn)
            self.hostname_in.insert(tkinter.END, parameters.get("HostnameIn"))
            self.content_url_in.insert(tkinter.END, parameters.get("ContentIn"))
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    def register_data(self):
        ...
        self.parameters_list.append(f"UsernameOut:{self.username_out.get()}")
        passwordOut = Hash().create_hash(self.password_out.get())
        self.parameters_list.append(f"PasswordOut:{passwordOut}")
        self.parameters_list.append(f"ServerURLOut:{self.server_url_out.get()}")
        self.parameters_list.append(f"ContentOut:{self.content_url_out.get()}")
        # Adicionando os parâmetros de Input do prep
        self.parameters_list.append(f"UsernameIn:{self.username_in.get()}")
        passwordIn = Hash().create_hash(self.password_in.get())
        self.parameters_list.append(f"PasswordIn:{passwordIn}")
        self.parameters_list.append(f"HostnameIn:{self.hostname_in.get()}")
        self.parameters_list.append(f"ContentIn:{self.content_url_in.get()}")
